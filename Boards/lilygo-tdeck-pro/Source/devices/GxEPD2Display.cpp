...
bool GxEPD2Display::startLvgl() {
    ...
    // Use internal RAM for the small persistent framebuffer to avoid SPIRAM TLSF allocator issues
...
void GxEPD2Display::lvglFlushCallback(lv_display_t* disp, const lv_area_t* area, uint8_t* px_map) {
    ...
    // stride must be based on hor_res (buffer width), not logical_w (area width)
    ...
    // Use internal RAM for the packed 1-bit buffer
    uint8_t* packed = (uint8_t*)heap_caps_malloc(packed_size, MALLOC_CAP_INTERNAL);
    if (!packed) {
        xSemaphoreGive(self->_framebufferMutex);
